...
func (c Claims) ValidateWithLeeway(e Expected, leeway time.Duration) error {
	...
	if c.NotBefore != nil && !e.Time.IsZero() && e.Time.Add(leeway).Before(c.NotBefore.Time()) {
		return ErrNotValidYet
	}

	if c.Expiry != nil && !e.Time.IsZero() && e.Time.Add(-leeway).After(c.Expiry.Time()) {
		return ErrExpired
	}
	...
	if c.IssuedAt != nil && !e.Time.IsZero() && e.Time.Add(leeway).Before(c.IssuedAt.Time()) {
