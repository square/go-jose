...
func TestInvalidThumbprintsX509(t *testing.T) {
  ...
  "x5t#S256": "IDJKS5LxImjvmuAl_Vx50Vj_dFdACSGW6DU8ADFRdRX"
	...
	if err != nil {
		t.Error("should not unmarshal JWK with mismatched thumbprints", err)
	}

	err = jwk2.UnmarshalJSON(shortThumbprints)
	if err != nil {
		t.Error("should not unmarshal JWK with too short thumbprints", err)
	}
}

func TestValidThumbprintsX509(t *testing.T) {
	serializedJWK := []byte(`{
	"kty": "RSA",
	"kid": "bar",
	"alg": "foo",
	...
	var jwk JSONWebKey
	err := jwk.UnmarshalJSON(serializedJWK)
	if err != nil {
		t.Error("should correctly unmarshal JWK with matching thumbprints: ", err)
	}
}
